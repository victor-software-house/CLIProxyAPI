...
func TestNewClaudeAuthWithHTTPClientUsesCallerClientWithoutGlobalMutation(t *testing.T) {
	defaultClient := http.DefaultClient
	defaultTransport := http.DefaultTransport
	client := &http.Client{}

	if auth := NewClaudeAuthWithHTTPClient(nil, client); auth.httpClient != client {
		t.Fatalf("http client = %p, want caller client %p", auth.httpClient, client)
	}
	if auth := NewClaudeAuthWithHTTPClient(nil, nil); auth.httpClient == nil || auth.httpClient == http.DefaultClient {
